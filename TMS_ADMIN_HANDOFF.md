...
## 1.7.0 Beta 2 온라인 업데이트 진행률 팝업

- 상단 로그인 상태를 덮어쓰던 업데이트 확인 문구를 제거하고 `온라인 업데이트` 모달을 추가했다.
- 조회·설치 준비는 불확정 진행 표시, 다운로드는 실제 바이트 비율 0~100%와 MB를 표시한다.
- 최신 버전·설치 확인 취소·조회/다운로드 오류 시 모달과 버튼을 복구한다. 업데이트 도구 실행 실패 시 로그인도 유지한다.
- 설치 파일 교체는 관리자 종료 후 기존 도구가 담당한다. DB, 발급키, 토큰 저장 방식은 변경하지 않았다.
- 기존 기능의 UI/오류 복구 보완으로 패치 버전 `1.7.0b2`를 적용했다.
- 다른 PC의 TMS 원격 변경은 `c36ebae` / `1.34.0b1`까지 확인했다. ICMP TLS·인제스트·SPL 로드·재생 제어와 CP950 음량 제어가 추가되어 있으며 실장비 검증은 별도다.
- 이 PC의 TMS 소스도 위 커밋으로 fast-forward 동기화했다. 오프라인 테스트 159개 중 158개 통과/1개 skip이며 HTTPError 정리 ResourceWarning은 남아 있다.
- 관리자 테스트 33개 통과. 운영 DB를 열지 않는 실제 Tk 팝업 생성·25% 표시·닫기 검증도 통과했다.

## 1.7.0 Beta 1 두 관리자 PC 공용 DB

- 기본 운영 DB를 NAS 동기화 전용 폴더 `License_DB/licenses.db`로 변경했다.
- 공용 DB 하나에 관리자 계정, 암호화된 발급 개인키, 공용 GitHub 업데이트 토큰, 발급 이력과 감사 로그를 저장한다.
- 이전 `data/licenses.db`만 있으면 다음 시작 시 SQLite 백업 API로 공용 폴더에 일관된 사본을 생성하며, 이전 DB는 로컬 복구용으로 남긴다.
- 두 PC에서 관리자를 동시에 실행하지 않는다. 한 PC에서 완전히 종료하고 NAS 동기화가 끝난 뒤 다른 PC에서 실행한다.
- `License_DB/`는 Git과 업데이트 패키지에 포함하지 않는다.
- 두 PC의 운영 이력 공유라는 신규 기능이므로 `MINOR`로 분류해 `1.7.0b1`로 올렸다.
