...
- 상단 계정 상태에 남던 업데이트 확인 문구를 제거하고 `온라인 업데이트` 팝업으로 분리했습니다.
- 버전 확인·설치 준비는 진행 중 표시, 다운로드는 실제 바이트 기준 0~100%와 MB 용량을 표시합니다.
- 최신 버전, 설치 확인 취소, 오류 시 팝업과 버튼 상태를 복구합니다. 설치 프로그램 실행 실패 시 로그인도 유지합니다.
- 실제 파일 교체는 관리자 종료 후 기존 업데이트 도구가 수행하며 DB와 설정은 유지합니다.
...
- 공용 읽기 토큰은 관리자 DB 암호화 저장과 장비별 라이선스 암호화에만 사용합니다.

## 라이선스 기반 공용 업데이트 토큰

- 공용 GitHub 읽기 전용 토큰은 관리자 DB에 로그인 발급키 기반 AES-GCM 암호문으로 저장됩니다.
- 새 `.tmshw` 파일의 장비별 X25519 공개키로 토큰을 다시 암호화하고, 서명된 라이선스 payload에 포함합니다.
- TMS 설치본과 소스에는 공용 토큰을 넣지 않으며 해당 장비의 개인키와 유효한 라이선스가 모두 있어야 복호화할 수 있습니다.
- 이전 `.tmshw` 파일은 읽을 수 있지만 자동 토큰 발급에는 최신 TMS에서 다시 저장한 파일이 필요합니다.
...
- 관리자 계정에서 `온라인 업데이트`를 사용할 수 있으며 별도의 GitHub 로그인이나 토큰 입력은 없습니다.
